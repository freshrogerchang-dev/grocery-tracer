...
- 某個商品**連續 3 次排程都抓取失敗**（網站擋爬蟲、頁面改版等），會額外發一則警告訊息告訴你，之後每 3 天最多提醒一次（避免像 Coupang 這種長期不穩的一直洗訊息）；抓取恢復正常後會自動清掉失敗計數。
- 每週一會發一則摘要訊息（目前追蹤幾個商品、有沒有正在打折的），讓你知道機器人還在正常運作，不需要額外設定。
- `/list`、`/price`、`/setprice`、`/setdiscount`、`/remove`、`/reset`、`/help`、`/search`、新增商品連結由 Cloudflare Worker 即時處理（見第 6、7 節），跟排程爬蟲互相獨立。
