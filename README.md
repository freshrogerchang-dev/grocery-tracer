...
- **Coupang 基本上抓不到**：已經確認 Coupang 的 WAF（Akamai）會直接回傳「Access Denied」擋掉自動化流量，商品頁跟搜尋頁都一樣，就算商品真的存在也讀不到內容。這是網站端的反爬蟲機制擋下來的，不是程式碼寫錯選擇器的問題，調整 `scrapers/coupang.py` 的解析邏輯沒有用。想解決要用住宅代理 IP 或瀏覽器指紋偽裝服務之類的東西，這個專案目前沒有做。建議不要太依賴 Coupang 的追蹤結果，或乾脆從 `watchlist.yaml` 移除。
...
- `/list` — 列出目前追蹤清單，附編號
- `/status` — 系統健康狀態：上次/下次排程執行時間、追蹤幾個商品、有沒有商品持續抓取失敗
...
- `/setprice <編號> <價格>` — 設定價格門檻，價格 <= 這個數字才通知（傳 `off` 取消門檻），例如 `/setprice 1 500`
- `/setdiscount <編號> <百分比>` — 設定折扣門檻，折扣 >= 這個百分比才通知（傳 `off` 取消門檻），例如 `/setdiscount 1 20`
- `/pause <編號>` — 暫停追蹤該筆，排程會跳過它但資料保留（例如已經買了，想留著之後比價）
- `/resume <編號>` — 恢復追蹤該筆
- `/remove <編號>` — 取消追蹤該筆（先傳 `/list` 看編號）。也可以用 `/remove <網址或關鍵字的一部分>`，例如 `/remove momoshop`，符合超過一筆時會請你改用編號
- `/remove all confirm` — 取消追蹤全部商品（跟 `/reset confirm` 是同一個功能，方便記）
- `/reset confirm` — 清空整個追蹤清單（先傳 `/reset` 或 `/remove all` 會先跟你確認一次，避免手滑）
- `/help` — 顯示指令說明
...
- 通知裡如果現在的價格是**目前記錄到的最低價**，會加上 🏆 標記（`/price` 查詢時也會顯示）。
- 某個商品**連續 3 次排程都抓取失敗**（網站擋爬蟲、頁面改版等），會額外發一則警告訊息告訴你，之後每 3 天最多提醒一次；抓取恢復正常後會自動清掉失敗計數。**Coupang 例外**：已確認是永久性的封鎖，不會再發這個警告（用 `/status` 查看仍看得到失敗次數）。
- 每週一會發一則摘要訊息（目前追蹤幾個商品、有沒有正在打折的、有沒有商品持續失敗），讓你知道機器人還在正常運作，不需要額外設定。
- 傳 `/pause <編號>` 暫停的商品，排程會完全跳過（不抓取、不計入失敗次數、不通知），直到你傳 `/resume` 恢復。
- `/list`、`/status`、`/price`、`/setprice`、`/setdiscount`、`/pause`、`/resume`、`/remove`、`/reset`、`/help`、`/search`、新增商品連結由 Cloudflare Worker 即時處理（見第 6、7 節），跟排程爬蟲互相獨立。
